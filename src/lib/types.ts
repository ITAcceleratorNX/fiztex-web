...
export interface Admin {
  ...
  fullName: string;
  /**
   * Роль аккаунта с бэкенда. Панель админская, но входят в неё и учителя, а разделы
   * `/api/admin/*` учительскому токену отвечают 401 — и общий `request()` считает это
   * концом сессии. Поэтому роль нужна хотя бы для того, чтобы не привести учителя на
   * экран, который его тут же разлогинит.
   */
  role?: string;
}
