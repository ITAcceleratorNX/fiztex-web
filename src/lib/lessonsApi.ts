...
export const lessonsApi = {
  ...
  /**
   * Свои уроки на сегодня — список раздела «Посещаемость (QR)». Ролевой адрес, а не
   * `/api/lessons?dateFrom=`: он отдаёт тот же набор полей, что неделя (класс, подгруппа,
   * кабинет, отмена), и отвечает статусом, если сегодня не учебный день.
   */
  myToday(signal?: AbortSignal): Promise<RoleSchedule> {
    return request<RoleSchedule>('/schedule/me/today', { signal });
  },
